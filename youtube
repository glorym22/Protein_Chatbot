...
def generate_response(query_text, vectorstore, callback):
    ...
    rag_prompt = [
        SystemMessage(
            content="너는 문서에 대해 질의응답을 하는 '리곰이'야. 주어진 논문과 문서를 참고하여 사용자의 질문에 답변을 해줘. 문서에 내용이 부족하다면 네가 알고 있는 지식을 포함해서 답변해줘"
            ),
        HumanMessage(
...
# page title
st.set_page_config(page_title='ʕ ᵒ ᴥ ᵒʔ 유튜브 영상 요약 및 QA 챗봇')
st.title('ʕ ᵒ ᴥ ᵒʔ The leelab \n 유튜브 영상 요약 및 QA 챗봇')
...
if "messages" not in st.session_state:
    st.session_state["messages"] = [
        ChatMessage(
            role="assistant", content="항상 수고가 많으십니다!"
        )
    ]
...
# message interaction
if prompt := st.chat_input("유튜브 링크를 입력해 주십쇼 ᕕʕ •ₒ• ʔ୨"):

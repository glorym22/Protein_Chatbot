...
def generate_response(query_text, vectorstore, callback):
    ...
    rag_prompt = [
        SystemMessage(
            content="너는 문서에 대해 질의응답을 하는 '서울대'야. 주어진 문서를 참고하여 사용자의 질문에 답변을 해줘. 문서에 내용이 정확하게 나와있지 않으면 너의 지식 선에서 잘 얘기해줘. 답변은 이모티콘을 넣어서 귀엽고 깜찍하게 해줘! 만약 대답하기 어려운 내용이 나온다면 너를 제작한 개발자가를 탓하도록 해"
        ),
        HumanMessage(
...
# page title
st.set_page_config(page_title='ก₍⸍⸌̣ʷ̣̫⸍̣⸌₎ค Theleelab 문서 기반 요약 및 QA 챗봇')
st.title('ก₍⸍⸌̣ʷ̣̫⸍̣⸌₎ค Theleelab 문서 기반 요약 및 QA 챗봇')
